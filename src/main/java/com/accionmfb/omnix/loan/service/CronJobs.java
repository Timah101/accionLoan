...
//    @Scheduled(fixedRate = 5000)
    private void sendSms() {
        ...
        List<Loan> loanByStatus = loanRepository.getAllLoanByStatus();

        if (loanByStatus != null) {
            log.info("SIZE OF STAGE2 LOANS {}", loanByStatus.size());
            for (Loan loanStatus : loanByStatus) {

                Loan loanUsingLoanId = loanRepository.getLoanUsingLoanId(loanStatus.getLoanId());

                loanUsingLoanId.setStatus("PENDING");

                loanRepository.updateLoan(loanUsingLoanId);

//                SMS newSMS = new SMS();
//                newSMS.setAppUser(loanStatus.getAppUser());
//                newSMS.setCreatedAt(LocalDateTime.now());
//                newSMS.setFailureReason("");
//                if (Objects.equals(loanStatus.getLoanAmountApproved(), BigDecimal.ZERO)) {
//                    newSMS.setMessage("Congratulations! Your SaveBrighta loan application with ACCION MFB has been approved. We are delighted to be able to assist you with your financial needs. For further details," +
//                            " please visit any of our closest branch or call us at [Customer Service Contact Details]\n" +
//                            "Accion Microfinance Bank");
//                } else {
//                    newSMS.setMessage("Dear Customer" +
//                            "We regret to inform you that your recent SaveBrighta  loan application with ACCION MFB has been declined. If you have any questions or would like further clarification regarding the decision, please contact our customer service at [Customer Service Contact Details]\n" +
//                            "Thank you for considering ACCION MFB and we wish you success in your financial endeavors.\n" +
//                            "Best regards,\n");
//                }
//
//                newSMS.setMobileNumber("09077927355");
//                newSMS.setRequestId("DERT45112W3ERE");
//                newSMS.setSmsFor("Loan Booking");
//                newSMS.setSmsType('N');
//                newSMS.setStatus("PENDING");
//                newSMS.setTimePeriod(genericService.getTimePeriod());
////                newSMS.setLoan(loanStatus);
//
//
//                SMSPayload oSMSPayload = new SMSPayload();
//                oSMSPayload.setMessage(newSMS.getMessage());
////                newSMS.getMobileNumber()
...
//
////                middlewareResponse = "";
//                System.out.println("Middleware response INSIDE CRON: " + middlewareResponse);
//                log.info("Middleware response INSIDE CRON: {}", middlewareResponse);
